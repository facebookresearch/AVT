...
def resize_hand(hand, target_size, interpolation):
  num_frames = hand.shape[0]
  tw, th = target_size
  resized_hand = np.zeros([num_frames, th, tw], dtype=hand.dtype)
  for idx in range(num_frames):
    resized_hand[idx, :, :] = cv2.resize(
      hand[idx, :, :], (tw, th), interpolation=interpolation)
  return resized_hand

def resize_hotspot(hotspot, target_size, interpolation):
  tw, th = target_size
  resized_hotspot = np.zeros([th, tw], dtype=hotspot.dtype)
  resized_hotspot= cv2.resize(hotspot, (tw, th), interpolation=interpolation)
  # print(resized_hotspot.shape)
...
class Compose(object):
  ...
  def __call__(self, clip, hand, hotspot):
    for t in self.transforms:

      clip, hand, hotspot = t(clip, hand, hotspot)
      # print(t)
      # print(clip.shape)
      # assert clip.shape[1] == hand.shape[1]
      # assert clip.shape[2] == hand.shape[2]
      # assert clip.shape[1] == hotspot.shape[0]
      # assert clip.shape[2] == hotspot.shape[1]

    return clip, hand, hotspot
...
class VideoScale(object):
  ...
  def __call__(self, clip, hand, hotspot):
    ...
    if isinstance(self.size, int):
      ...
      resized_clip = resize_clip(clip, (ow, oh), interpolation=interpolation)
      resized_hand = resize_hand(hand, (ow, oh), interpolation=interpolation)
      resized_hotspot = resize_hotspot(hotspot, (ow, oh), interpolation=interpolation)
      return resized_clip, resized_hand, resized_hotspot
    else:
      h, w = clip.shape[1], clip.shape[2]
...
class VideoRandomRotate(object):
  ...
  def __call__(self, clip, hand, hotspot):
    ...
    rotated_clip = np.zeros([clip.shape[0], hr, wr, clip.shape[3]],
                            dtype=clip.dtype)
    rotated_hand = np.zeros([hand.shape[0], hr, wr],dtype=hand.dtype) 
    rotated_hotspot = np.zeros([hr, wr], dtype=hotspot.dtype)      
    ...
    for idx in range(clip.shape[0]):
      rotated_clip[idx, :, :, :] = cv2.warpAffine(clip[idx, :, :, :],
                                                  ...
                                                  (wr, hr),
                                                  flags=interpolation)
    for idx in range(hand.shape[0]):
      rotated_hand[idx, :, :] = cv2.warpAffine(hand[idx, :, :],
                                                  rot_mat,
                                                  (wr, hr),
                                                  flags=interpolation)      
    rotated_hotspot = cv2.warpAffine(hotspot,rot_mat,
                                              (wr, hr),
                                              flags=interpolation)      
    # print(rotated_clip.shape)
    # print(rotated_hand.shape)
    # print(rotated_hotspot.shape)

    return rotated_clip, rotated_hand, rotated_hotspot
...
class VideoCenterCrop(object):
  ...
  def __call__(self, clip, hand, hotspot):
    ...
    y1 = int(round((h - th) / 2.))
    clip = clip[:, y1 : y1 + th, x1 : x1 + tw, :]
    hand = hand[:, y1 : y1 + th, x1 : x1 + tw]
    hotspot = hotspot[y1 : y1 + th, x1 : x1 + tw]
...
class VideoRandomMultiScaleCrop(object):
  ...
  def __call__(self, clip, hand, hotspot):
    ...
    cropped_hand = hand[:,offset_h:offset_h+crop_h, offset_w:offset_w+crop_w]
    cropped_hotspot = hotspot[offset_h:offset_h+crop_h, offset_w:offset_w+crop_w]
    # print(cropped_clip.shape)
    resized_clip = resize_clip(cropped_clip, self.input_size, interpolation=interpolation)

    resized_hand = resize_hand(cropped_hand, self.input_size, interpolation=interpolation)

    resized_hotpsot = resize_hotspot(cropped_hotspot, self.input_size, interpolation=interpolation)
    # print(resized_hotpsot.shape)

    return resized_clip, resized_hand, resized_hotpsot
  ...
  def __call__(self, clip, hand, hotspot):
    ...
    if h <= w:
      ...
      for x1 in all_x1:
        ...
        hand_crop = hand[:, y1 : y1 + th, x1 : x1 + tw]

        hotspot_crop = hotspot[y1 : y1 + th, x1 : x1 + tw]
        clips.append(clip_crop[np.newaxis, :, :, :, :])
        hands.append(hand_crop[np.newaxis, :, :, :])
        hotspots.append(hotspot_crop[np.newaxis, :, :])
    # crop along height
    else:
      x1 = int(round((w - tw) / 2.))
      if self.num_crops == 1:
    ...
    hotspots = np.concatenate(hotspots, axis=0)

    return clips, hands,hotspots
...
################################################################################
def create_video_transforms_joint(input_config):
