...
def range_to_frame_rep(durations, labels, fps):
    """
    Args:
        durations (list): list of durations in seconds, for each label
        label (list): The label for the corresponding duration
    Returns:
        frame level labels
    """
    ...
    for duration, label in zip(durations, labels):
        if duration <= 0:
            continue
        res += [int(label)] * int(round(duration * fps))
    return res


def compute_moc(gt_cls, pred_cls, num_classes):
    """
    As per Abu Farha CVPR'18; copied over from
    https://github.com/yabufarha/anticipating-activities/blob/master/eval.py
    Args:
        gt_cls: List of GT labels
        pred_cls: List of predicted labels corresponding to the same frame
    Returns:
        a mean over classes (MOC) metric
    """
    num_pos = np.zeros((num_classes, ))
    num_neg = np.zeros((num_classes, ))
    null_labels = 0
    for gt, pred in zip(gt_cls, pred_cls):
        if gt == -1:
            null_labels += 1
            continue
        elif gt == pred:
            ...
            num_neg[gt] += 1
    print(f'%age of -1 labels: '
          f'{(null_labels / (num_pos.sum() + num_neg.sum())) * 100:.2f}%')
    acc = 0
    tot = 0
    for i in range(num_classes):
        if num_pos[i] + num_neg[i] != 0:
            acc += float(num_pos[i]) / (num_pos[i] + num_neg[i])
            tot += 1
    return acc / tot


def compute_dense_moc(dataset, res_action):
    res_action_cls = get_logits_from_results(res_action).argmax(axis=-1)
    res_action_len = res_action['logits_regression']
    all_gt_preds_by_percs = {}
    for i in tqdm(range(len(dataset)), desc='Range to frame labels'):
        assert dataset[i]['target'][-1, 1] == -1, (
            'To ensure the target rollout contains all labeled segments, '
            'This just checks the last segment is -1 => it was padded => '
            'it contains all the labeled segments.')
        gt = range_to_frame_rep(
            dataset[i]['target'][:, 0].reshape((-1, )).tolist(),
            dataset[i]['target'][:, 1].reshape(
                (-1, )).tolist(), dataset[i]['video_info']['video_fps'])
        pred = range_to_frame_rep(res_action_len[i].reshape(
            (-1, )).tolist(), res_action_cls[i].reshape((-1, )).tolist(),
                                  dataset[i]['video_info']['video_fps'])
        # Keep pred only until gt is, if smaller, pad with last class
        pred = pred[:len(gt)]
        if len(pred) < len(gt):
            last_pred_cls = pred[-1] if len(pred) > 0 else 0
            pred += [last_pred_cls] * (len(gt) - len(pred))
        # Round out the obs_perc to 2 digits
        obs_perc = (
            int(round(dataset[i]['end'] / dataset[i]['orig_vid_len'] * 100)) /
            100)
        pred_percs = [0.1, 0.2, 0.3, 0.5]
        for pred_perc in pred_percs:
            num_pred_frames = int((pred_perc / (1 - obs_perc)) * len(gt))
            if obs_perc not in all_gt_preds_by_percs:
                all_gt_preds_by_percs[obs_perc] = {}
            if pred_perc not in all_gt_preds_by_percs[obs_perc]:
                # Make 2 lists, first for GT, and second for predictions
                all_gt_preds_by_percs[obs_perc][pred_perc] = [[], []]
            all_gt_preds_by_percs[obs_perc][pred_perc][
                0] += gt[:num_pred_frames]
            all_gt_preds_by_percs[obs_perc][pred_perc][
                1] += pred[:num_pred_frames]
    # Compute MOC for each setting
    all_moc_by_percs = {}
    for obs_perc in all_gt_preds_by_percs:
        all_moc_by_percs[obs_perc] = {}
        for pred_perc in all_gt_preds_by_percs[obs_perc]:
            all_moc_by_percs[obs_perc][pred_perc] = compute_moc(
                *all_gt_preds_by_percs[obs_perc][pred_perc],
                num_classes=len(dataset.classes))
    # Print out the MOC
    final_res = {}
    for obs_perc in all_moc_by_percs:
        pred_percs = sorted(all_moc_by_percs[obs_perc].keys())
        print(f'Obs perc: {obs_perc}, pred_percs: {pred_percs}')
        print(' '.join([
            '%.1f' % (all_moc_by_percs[obs_perc][pred_perc] * 100)
            for pred_perc in pred_percs
        ]))
        for pred_perc in pred_percs:
            final_res[(obs_perc,
                       pred_perc)] = all_moc_by_percs[obs_perc][pred_perc]
    return final_res


def compute_dense_moc_all(run_info, datasets=None):
    res_actions = read_all_results(*run_info)
    results = {}
    for key, res_action in res_actions.items():
        print(f'Evaluating results dir: {key}')
        key_suffix = key[len(RESULTS_SAVE_DIR_PREFIX):]
        if datasets is None:
            dataset = get_dataset(*run_info,
                                  dataset_cfg_key=(DATASET_EVAL_CFG_KEY +
                                                   key_suffix))
        else:
            dataset = datasets[key_suffix]
        results[key_suffix] = compute_dense_moc(dataset, res_action)
    return results


def visualize_gpt_loss_videos(run_info,
                              dataset_key_suffix=DATASET_EVAL_CFG_KEY_SUFFIX):
    feats = read_results(*run_info, results_dir=f'results{dataset_key_suffix}')
    gpt_loss = feats['loss/gpt2']
    dataset = get_dataset(*run_info, dataset_key_suffix=dataset_key_suffix)
    # Assume this is the dense dataset, to take only the ones with -1 labels
    # which means those are the "dense" parts added
    dense_df = dataset.df[dataset.df.action_class == -1]
    assert len(dense_df) > 0
    for video_id in dense_df.video_id.unique():
        data_subset = dense_df[dense_df.video_id == video_id]
        # For each of the rows, find the loss from GPT
        loss_values = gpt_loss[data_subset.index]
        # Take the last one, and mean over the dimensions
        loss_values = loss_values[:, -1, :].mean(axis=-1)
        plt.plot(loss_values)
        plt.show()
        # TODO: Need to visualize over the video actually
